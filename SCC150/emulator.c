...
int exec_bytecode() {
  ...
  while (text[TEXT_POS(pc)] != 0)
  {
    ...
    // ADD
    else if ((text[TEXT_POS(pc)] & 0xFC0007FF) == 0x00000020) 
    {
			int s = (text[TEXT_POS(pc)] & 0x03E00000) >> 21;
			int t = (text[TEXT_POS(pc)] & 0x001F0000) >> 16;
    ...
    // SRL
    else if ((text[TEXT_POS(pc)] & 0xFC00003F) == 0x00000002) 
    {
			int t = (text[TEXT_POS(pc)] & 0x001F0000) >> 16;
			int d = (text[TEXT_POS(pc)] & 0x0000F800) >> 11;
			int h = (text[TEXT_POS(pc)] & 0x000007C0) >> 6;
			registers[d] = registers[t] >> h;
			pc = pc + 4;
	}
		
    // SLL
    else if ((text[TEXT_POS(pc)] & 0xFC00003F) == 0x00000000) 
    {
			int t = (text[TEXT_POS(pc)] & 0x001F0000) >> 16;
			int d = (text[TEXT_POS(pc)] & 0x0000F800) >> 11;
			int h = (text[TEXT_POS(pc)] & 0x000007C0) >> 6;
